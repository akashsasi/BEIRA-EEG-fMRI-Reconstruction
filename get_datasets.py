...
def interpolate_df_eeg_fmri(df_eeg, df_fmri):
    ...
    Then crop by fMRI length of recording. Estimate fps of EEG data and return it.
    ...
    # Ensure time columns are in milliseconds
    if df_eeg['time'].iloc[-1] < 1000:  # Assuming time is in seconds
        df_eeg['time'] *= 1000

    if df_fmri['time'].iloc[-1] < 1000:  # Assuming time is in seconds
        df_fmri['time'] *= 1000

    # Debug print to check time ranges
    print(f"EEG time range: {df_eeg['time'].min()} to {df_eeg['time'].max()}")
    print(f"fMRI time range: {df_fmri['time'].min()} to {df_fmri['time'].max()}")
    ...
    # Get indices of EEG that correspond with fMRI time range
    filter_rule = (df_eeg['time'] >= df_fmri['time'].iloc[0]) & (df_eeg['time'] <= df_fmri['time'].iloc[-1])
    df_eeg_cropped = df_eeg[filter_rule]

    # Check if there are overlapping time points
    if df_eeg_cropped.empty:
        print("No overlapping time points between EEG and fMRI data.")
        return df_eeg_cropped, pd.DataFrame(), 0

    # Interpolate fMRI data for the cropped EEG times
    fmri_interpolate = fmri_interpolate_func(df_eeg_cropped['time'])
    df_fmri_interp = pd.DataFrame(fmri_interpolate, columns=df_fmri.columns)

    # Get fps value of signals
    if len(df_eeg_cropped) >= 2:
        original_fps = 1 / (df_eeg_cropped['time'].iloc[1] - df_eeg_cropped['time'].iloc[0]) * 1000
    else:
        original_fps = 100  # or some other default value

    return df_eeg_cropped, df_fmri_interp, int(original_fps)
...
def download_cwl_dataset(patient, path_to_dataset, 
    ...
    return df_eeg, df_fmri, df_fmri.drop('time', axis=1).columns.to_list()
